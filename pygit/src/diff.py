...
def diff_trees(t_from: Dict[str, str], t_to: Dict[str, str]) -> str:
    output = b""
...
def diff_blobs(o_from: str, o_to: str, path="blob"):
...
) -> Generator[tuple[str, Literal["new file", "deleted", "modified"]], Any, None]:
    for path, o_from, o_to in compare_trees(t_from, t_to):
        if o_from != o_to:
...
def merge_trees(t_base: str, t_HEAD: str, t_other: str) -> defaultdict[str, bytes]:
    tree = defaultdict(bytes)
...
def merge_blobs(o_base: str, o_HEAD: str, o_other: str) -> bytes:
